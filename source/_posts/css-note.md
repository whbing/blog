...
## CSS定位
### 普通流
相对定位，相对于元素在文档流中的初始位置定位，元素仍然占据原来的空间。
### 绝对定位
相对于距离最近的已定位的祖先元素，若不存在，那么相对于初始包含块。
不占据空间，和文档普通流无关。可以覆盖页面上的其他元素，可以通过`z-index`属性控制这些框的叠放顺序。

固定定位：相对于视口的绝对定位。
### 浮动和清除浮动
浮动可以左右移动，直到它的外边缘碰到包含框或另一个浮动框的边缘。
浮动框不在文档的普通流中。

使用`clear`清除浮动，对元素进行清除浮动实际上是为前面的浮动元素留出了垂直空间。
